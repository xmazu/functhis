...
describe('validateContractInput', () => {
  ...
  test('validates boolean, array, and object types', () => {
    expect(validateContractInput({ type: 'boolean' }, true)).toEqual({
      ok: true,
    });
    expect(validateContractInput({ type: 'boolean' }, 'yes').ok).toBe(false);
    expect(validateContractInput({ type: 'array' }, [])).toEqual({ ok: true });
    expect(validateContractInput({ type: 'array' }, {}).ok).toBe(false);
    expect(
      validateContractInput({ items: { type: 'number' }, type: 'array' }, [
        1,
        'x',
      ]).ok
    ).toBe(false);
    expect(validateContractInput({ type: 'object' }, []).ok).toBe(false);
    expect(validateContractInput({ type: 'object' }, {}).ok).toBe(true);
    expect(validateContractInput({ type: 'integer' }, 1)).toEqual({ ok: true });
  });
});
