    ...
    >
      ...
      <div className="mt-5 flex flex-col gap-2">
        {DEMO_FUNCTIONS.map((fn) => {
          const open = previewSlug === fn.slug;
          return (
            <button
              aria-controls={previewId}
              aria-expanded={open}
              className={cn(
                'w-fit max-w-full cursor-pointer truncate text-left font-mono text-[12px] transition-colors duration-[120ms]',
                open ? 'text-zinc-50' : 'text-zinc-500 hover:text-zinc-300'
              )}
              key={fn.slug}
              onClick={() => {
                openPreview(fn.slug);
      ...
      <div className={preview ? 'mt-4' : 'hidden'} id={previewId}>
